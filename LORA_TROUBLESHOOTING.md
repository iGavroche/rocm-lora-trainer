...
- Updated `generate_wan22_14B_i2v.sh` to use `--task i2v-A14B`
...
- Updated LoRA path to `wan22_14B_i2v_full.safetensors` (the I2V-trained LoRA)

**Option 2: Retrain for T2V**
If you prefer text-to-video generation:
1. Update `train_wan22_14B_i2v_full.sh` to use `--task t2v-A14B`
...
1. Generate with the updated script: `./generate_wan22_14B_i2v.sh`
...
- `generate_wan22_14B_i2v.sh`: Fixed to use I2V task matching the training
- `train_wan22_14B_i2v_full.sh`: Already correctly configured for I2V training
